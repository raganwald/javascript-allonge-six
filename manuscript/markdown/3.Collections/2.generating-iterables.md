...
[^empty]: We wrote a *generator declaration*. We can also write `const empty = function * () {}` to bind an anyonymous generator to the `empty` keyword, but we don't need to do that here.
