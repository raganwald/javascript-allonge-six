We've [previously](#stateful-method-decorators) looked at method decorators like thi:

 and ES.later[^ESdotlater] method decorators like this:
...
We also saw that if our tooling supports ES.later[^ES.later] decorators, we can write:
...
Mind you, there's always room for polish and gold plate. We could enhance `beforeAll`, `afterAll`, and `around` to include conveniences like regular expressions to match method names, or special declarations like `except:` or `only:` if we so desired.
