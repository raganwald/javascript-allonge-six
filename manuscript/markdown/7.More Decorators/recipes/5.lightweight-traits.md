...
A trait is like a mixin, however with a trait, we can not just define behaviour, but also define ways to extend or even override behaviour. Traits are a first-class feature of some languages, but we can make lightweight traits out of the receipes we already have.
