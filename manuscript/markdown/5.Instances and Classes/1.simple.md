...
What's this `prototype` property? Let's run our standard test:
...
That's very interesting! Instances seem to behave as if they *delegate* to their constructors prototype, just as if we'd created them using `Object.create(Ur.prototype)`. Let's try a few things:
...
Even more interesting: Changing the constructor's prototype changes the behaviour of all of its instances. This *is* the prototype relationship we have already seen with `Object.create`.
...
Roughly speaking, we could use `Object.create` to emulate the `new` keyword, and use `new` to emulate `Object.create`.
