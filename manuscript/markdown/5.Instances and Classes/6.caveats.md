...
There are techniques for encapsulating prototypes so that the coupling between classes in a class hierarchy is reduced, and we should use them wherever possible. We should also avoid hierarchies in the first place, using techniques such as mixins rather than building deep chains.