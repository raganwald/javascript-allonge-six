...
We're privately extending a domain object. When we call `assignPrivately`, a new object is created to hold the private state. We call it once for each object, so each object has its own private state for `HasCareer`

Now let's try it with a shared prototype:

~~~~~~~~
const Careerist = Object.create(null);
assignPrivately(Careerist, HasCareer);
...
Let's rewrite our `assignPrivately` function to store the private data in safekeeping:

~~~~~~~~
let number = 0;

function safeAssignPrivately (prototype, mixin) {
...
safeAssignPrivately(Careerist, HasName);
safeAssignPrivately(Careerist, HasCareer);
...
safeAssignPrivately(coder, HasName);
safeAssignPrivately(coder, HasCareer);
